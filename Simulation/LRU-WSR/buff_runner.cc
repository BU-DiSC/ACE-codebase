...
void Enqueue(EmuEnv* _env, Queue *queue, Hash *hash, unsigned pageId, bool isWrite, int algorithm)
{
  ...
  {
    // *******************************
    // New addition compared to LRU
    QNode *iterator_tail;
    while (1)
    {
      iterator_tail = queue->rear;
      if (iterator_tail->dirty)
      {
        if (iterator_tail->cold)
          break;
        else // move the tail to the front
        {
          iterator_tail->prev->next = NULL;
          queue->rear = iterator_tail->prev;
          queue->front->prev = iterator_tail;
          iterator_tail->next = queue->front;
          iterator_tail->prev = NULL;
          queue->front = iterator_tail;
          queue->front->cold = true;
        }
      }
      else
        break;
    ...
    if (algorithm == 1) // Classical LRU-WSR
    {
    ...
    else if (algorithm == 2) // CONE-a
    {
      if (queue->rear->dirty)   // Evict multiple pages ONLY if you are about to evict a dirty page
      {
        ...
        while (alpha_window < _env->concurrency)    // Sending all the candidate pages of the alpha_window to the rear of the queue. (To use the 'dequeue' method)
        {
          if (moving_tail->dirty)
          {
            ...
            else    // move it to the front
            {
              ...
              moving_tail->prev->next = moving_tail->next;
              moving_tail->next->prev = moving_tail->prev;
              tempq->front->prev = moving_tail;
              moving_tail->next = tempq->front;
              moving_tail->prev = NULL;
              tempq->front = moving_tail;
              tempq->front->cold = true;
    ...
    else if (algorithm == 3) // CONE-Xa
    {
      if (queue->rear->dirty) // Evict multiple pages ONLY if you are about to evict a dirty page
      {
        ...
        while (eligible_for_eviction_count < _env->concurrency) // Sending all the candidate pages in the alpha_window to the rear of the queue. (To use the 'dequeue' method)
        {
          if (moving_tail->dirty)
          {
            if (moving_tail->cold)
        ...
        while (alpha_window < _env->concurrency)    
        {
          ...
          if (moving_tail->dirty)
          {
            ...
            else
            {
              ...
              moving_tail->prev->next = moving_tail->next;
              moving_tail->next->prev = moving_tail->prev;
              queue->front->prev = moving_tail;
              moving_tail->next = queue->front;
              moving_tail->prev = NULL;
              queue->front = moving_tail;
              queue->front->cold = true;
...
{
  QNode* moving_head = buffer_instance->queue->front;
  ...
  moving_head = buffer_instance->queue->front;
  while (moving_head)
  {
    if (moving_head->dirty)
