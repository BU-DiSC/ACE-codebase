...
int main(int argc, char *argvx[])
{
  ...
  {
    std::cerr << "Generating workload ... " << std::endl << std::flush;  
    WorkloadGenerator workload_generator;
    workload_generator.generateWorkload();
    gettimeofday(&(time1), NULL);
    std::cerr << "Executing workload ... " << std::endl << std::flush;  
...
int parse_arguments(int argc,char *argvx[], EmuEnv* _env) {
  ...
  args::ValueFlag<int> window_cmd(group1, "W", "Window size of CFLRU [def:2]", {'W', "window"});
  ...
  _env->window = window_cmd ? args::get(window_cmd) : 2;
...
void printEmulationOutput(EmuEnv* _env) {
  //if (_env->verbosity >= 1) 
  std::cout << "b\t n\t x\t e\t e'\t a\t s\t d\t r\t w\t p\t k\t W  \n";
